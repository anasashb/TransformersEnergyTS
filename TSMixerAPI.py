...
class TSFDataLoader:
    ...
    def _read_data(self):
        ...
        n = len(df)
        # NOTE Splits to match Torch transformers
        if self.data_path.startswith("ETTh"):
            train_end = 12 * 30 * 24
...
class TSMixer:
    ...
    def __init__(self, model="tsmixer_rev_in"):
        self.args = dotdict()
        self.args.model = model  # NOTE to handle ablation
        self.args.seed = 100
        ...
        ## Variables for TS
        self.args.seq_len = 168  # NOTE default seq_len in our exps. Original uses 336.
        # Model Architecture
        ...
        self.args.itr = 3  # how many experiment rounds
    ...
    def fit(
        ...
        if pred_len not in possible_predlens:
            # NOTE deacivate error to use more
            raise ValueError(
                "Prediction length outside current experiment scope. Please use either 24, 48, 96, 168, 336, 720."
            )
        self.args.data = data  # NOTEcould be redundant
