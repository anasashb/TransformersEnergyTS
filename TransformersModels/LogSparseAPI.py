...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        _path = os.path.join(self.root_path, self.data_path).replace(
            "\\", "/"
        )  # replace in case windows
...
class DataEmbedding(nn.Module):
    def __init__(
        ...
        self.position_embedding = (
            PositionalEmbedding(d_model=self.d_pos) if pos_embed else None
        )
        if temp_embed:
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        if self.sparse_flag:
            sparse_mask = LogSparseMask(
                B, L, S, self.win_len, self.res_len, device=queries.device
            )
            if self.mask_flag:
...
class AttentionLayer(nn.Module):
    def __init__(
        self, attention, d_model, n_heads, d_keys=None, d_values=None, output_size=None
    ):
        ...
        self.value_projection = nn.Linear(d_model, d_values * n_heads)
        self.out_projection = (
            nn.Linear(d_values * n_heads, d_model)
            if output_size is None
            else nn.Linear(d_values * n_heads, output_size)
        )
        self.n_heads = n_heads
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, **kwargs):
        ...
        for layer in self.layers:
            x, a_sa, a_ca = layer(
                x, cross, x_mask=x_mask, cross_mask=cross_mask, **kwargs
            )
            attn.append(a_sa)
            attn.append(a_ca)
...
def adjust_learning_rate(optimizer, epoch, args, warmup=0):
    ...
    if args.lradj == "type1":
        if epoch < warmup:
            lr_adjust = {
                epoch: epoch * (args.learning_rate - args.learning_rate / 100) / warmup
                + args.learning_rate / 100
            }
        else:
            lr_adjust = {
                epoch: args.learning_rate
                * (args.lr_decay_rate ** ((epoch - warmup) // 1))
            }  # 0.5
        ...
        lr_start = (
            args.learning_rate
            * decay_rate1 ** ((epoch + period) // period)
            / decay_rate1
        )
        lr_end = (
            args.learning_rate
            * decay_rate2 ** ((epoch + period * 2) // period)
            / decay_rate2
        )
        lr_adjust = {
            epoch: (0.5 + 0.5 * np.cos(np.pi / period * (epoch % period)))
            * (lr_start - lr_end)
            + lr_end
        }
...
class EarlyStopping:
    def __init__(
        self, patience=7, verbose=False, delta=0, checkpoint=True, model_setup=None
    ):
    ...
    def save_checkpoint(self, val_loss, model, path, epoch):
        ...
        pickle.dump(
            self.model_setup.__dict__, open(path + "/" + "model_setup.pickle", "wb")
        )
        pickle.dump(
            {"epoch": epoch, "val_loss_min": val_loss, "val_losses": self.val_losses},
...
class Exp_Logsparse(Exp_Basic):
    ...
    def vali(
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                # decoder input
                dec_zeros = dec_inp[
                    :, (self.args.label_len - 1) : self.args.label_len, :
                ]  # Select last value
                dec_zeros = dec_zeros.repeat(
                    1, self.args.pred_len, 1
                ).float()  # Repeat for pred_len
                dec_inp = torch.cat(
                    [dec_inp[:, : self.args.label_len, :], dec_zeros], dim=1
                )  # Add Placeholders
    ...
    def train(self, setting):
        ...
        if self.args.checkpoint_flag:
            ...
            if os.path.exists(load_path) and self.load_check(
                path=os.path.normpath(os.path.join(path, "model_setup.pickle"))
            ):
                ...
                epoch_info = pickle.load(
                    open(
                        os.path.normpath(
                            os.path.join(
                                "./checkpoints/" + setting, "epoch_loss.pickle"
                            )
                        ),
                        "rb",
                    )
        ...
        for epoch in range(start_epoch, self.args.train_epochs):
            ...
            if self.args.model == "LSTM":
                ...
                teacher_forcing_ratio = max(0.0, teacher_forcing_ratio)
                ...
                if (i + 1) % 100 == 0 and self.args.verbose == 1:
                    print(
                        "\titers: {0}/{1}, epoch: {2} | loss: {3:.7f}".format(
                            i + 1, num_iters, epoch + 1, np.average(train_loss)
                        )
                    )
            ...
            vali_loss = self.vali(
                setting, vali_data, vali_loader, criterion, epoch=epoch, save_path=path
            )
            test_flag = False
            if test_flag:
                test_loss = self.vali(
            ...
            if self.args.plot_flag and self.args.checkpoint_flag:
                ...
                if "fig_progress" not in locals():
                    fig_progress = PlotLossesSame(
                        epoch + 1, Training=train_loss, Validation=vali_loss
                    )
                else:
                    fig_progress.on_epoch_end(Training=train_loss, Validation=vali_loss)
                ...
                pickle.dump(
                    fig_progress, open(loss_save_dir_pkl, "wb")
                )  # To load figure that we can append to
    ...
    def test(
        ...
        if test:
            ...
            if len(base_dir) == 0:
                ...
                load_path = os.path.normpath(
                    os.path.join(base_dir + "checkpoints/" + setting, "checkpoint.pth")
                )
            load_check_flag = self.load_check(
                path=os.path.normpath(
                    os.path.join(os.path.dirname(load_path), "model_setup.pickle")
                ),
        ...
        if self.args.inverse:
            ...
            np.save(
                folder_path + "metrics_un.npy",
                np.array([mae_un, mse_un, rmse_un, mape_un, mspe_un]),
            )
...
class LogsparseTS:
    ...
    def __init__(self, model="Logsparse"):
        ...
        self.args.model_id = (
            self.args.model + "_" + str(self.args.data) + "_" + str(self.args.pred_len)
        )
        # Optimization
    ...
    def fit(
        ...
        self.setting = "{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_dl{}_df{}_fc{}_eb{}_dt{}_des{}_iter{}".format(
            self.args.model,
            self.args.data,
            self.args.features,
            self.args.seq_len,
